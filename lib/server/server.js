...
/**
 * Starts the server based on the provided options.
 * @param {Object} options - The server options.
 * @param {Object} options.serverConfig - The server configuration.
 * @param {Object} options.uiConfig - The UI configuration.
 * @returns {boolean} - Returns false if the server is not enabled, otherwise undefined.
 */
export const startServer = async (options) => {
  const { serverConfig, uiConfig } = options;
  ...
  // Set up static folder's route (if ui is enabled)
  const uiEnabled = uiConfig?.enable;
  if (uiEnabled) {
    app.use(
      uiConfig?.route || '/',
      express.static(posix.join(__dirname, 'public'))
    );
  }
